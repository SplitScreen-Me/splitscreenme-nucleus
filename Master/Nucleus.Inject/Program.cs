...
namespace Nucleus.Inject
{
    internal class Program
    {
        ...
        private static void Main(string[] args)
        {
            ...
            string[] argsDecoded = new string[args.Length];
        ...
        private static void StartupHook(string[] args, int i, bool is64)
        {
            ...
            byte[] logPath = Encoding.Unicode.GetBytes(nucleusFolderPath);
            int logPathLength = logPath.Length;

            byte[] targetsBytes = Encoding.Unicode.GetBytes(mutexToRename);
            int targetsBytesLength = targetsBytes.Length;

            byte[] rawHidBytes = Encoding.Unicode.GetBytes(rawHid);
            int rawHidBytesLength = rawHidBytes.Length;

            byte[] playerNickBytes = Encoding.Unicode.GetBytes(playerNick);
        ...
        private static void RuntimeHook(string[] args, int i, bool is64)
        {
            ...
            byte[] writePipeNameBytes = Encoding.Unicode.GetBytes(writePipeName);
            int writePipeNameLength = writePipeNameBytes.Length;

            byte[] readPipeNameBytes = Encoding.Unicode.GetBytes(readPipeName);
